    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :categoryImg="skuInfo || [{}]" />
          <!-- 小图列表 -->
          <ImageList :categoryImg="skuInfo || [{}]" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuList in spuSaleAttrList" :key="spuList.id">
                <dt class="title">{{ spuList.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: spuValueList.isChecked == 1 }"
                  v-for="spuValueList in spuList.spuSaleAttrValueList"
                  :key="spuValueList.id"
                  @click="clckValue(spuValueList,spuList.spuSaleAttrValueList)"
                >
                  {{ spuValueList.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNumber"/>
                <a href="javascript:" class="plus" @click="skuNumber++">+</a>
                <a href="javascript:" class="mins" @click="skuNumber>0?skuNumber--:skuNumber=1">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods:{
    clckValue(spuValueList,arr){
      arr.forEach(item => {
        item.isChecked = '0'      
      })
      spuValueList.isChecked ='1'
    },
    async addShopCar(){
        try{
          await this.$store.dispatch("addShoppingCar",{skuId:this.$route.params.skuid,skuNum:this.skuNumber})
          sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
          this.$router.push({path:"/addcartsuccess",query:{skuNum:this.skuNumber}})
        }catch(error){
