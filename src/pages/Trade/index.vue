<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(userAddress) in userAddressList" :key="userAddress.id" @click="changDefult(userAddress,userAddressList)">
      ...
      <div class="detail" >
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="detailList in orderInfo.detailArrayList" :key="detailList.skuId">
          <li>
          ...
          <li>
            <h3>￥{{detailList.orderPrice}}.00</h3>
          </li>
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <textarea placeholder="建议留言前先与商家沟通确认" class="remarks-cont" v-model="conts"></textarea>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{getChangDefult.fullAddress}}</span>
        收货人：<span>{{getChangDefult.consignee}}</span>
        <span>{{getChangDefult.phoneNum}}</span>
      </div>
    </div>
...
import {reqGetSubmitOrder} from '@/api'
  export default {
    ...
    methods:{
      changDefult(userAddress,userAddressList){
      ...
      async goPay(){
        ...
        let data ={
          consignee: this.getChangDefult.consignee,
          consigneeTel: this.getChangDefult.phoneNum,
          deliveryAddress: this.getChangDefult.fullAddress,
          paymentWay: 'ONLINE',
          orderComment: this.conts,
          orderDetailList: this.orderInfo.detailArrayList
        }
        let result = await reqGetSubmitOrder(tradeNo,data)
        console.log(result)
        if(result.code==200){
